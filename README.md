...
- 在旅游路线列表或路线详情中可以为已保存路线生成只读分享链接。
- 分享页只展示路线名称、城市、出行方式、地图路线、地点顺序、备注和图片，不提供保存、删除、上传图片、切换候选或智能排序等编辑能力。
- 同一条路线重复生成分享链接时会复用已有 token，不会反复创建多个分享地址。
- 删除原路线后，分享链接会自然失效。
- 当前分享链接不支持密码、过期时间和权限控制。
- 生产环境需要配置正确的前端访问域名；本地生成的 `localhost` 分享链接只适合本机测试。
